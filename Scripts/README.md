# Scripts

This folder contains the R scripts used for data processing, statistical analyses, and the production of figures for this study.

## R packages

The following R packages were used:

- `dplyr`
- `tidyr`
- `stringr`
- `ggplot2`
- `glmmTMB`
- `MASS`
- `car`
- `emmeans`
- `DHARMa`
- `vegan`
- `bipartite`
- `tibble`
- `ggtext`

## Terminology

Some terminology within the scripts reflects naming conventions established during the initial data collection and analysis and has therefore been retained for consistency.

In particular:

- **Pollinators** refers to **floral visitors**. Visitation was measured, rather than successful pollination.
- **Native species** refers to **non-restored plant species**. This terminology does not indicate or assess the native status of individual plant species.

These distinctions should be considered when interpreting variable names, objects, comments, and outputs within the scripts.
